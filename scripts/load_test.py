...
def _current_rate(t: float, peak: float, ramp_up: float, duration: float, ramp_down: float) -> float:
...
def _total_expected(t: float, peak: float, ramp_up: float, duration: float, ramp_down: float) -> float:
    """
...
def main() -> None:
    ...
    parser.add_argument("--ramp-down", type=float, default=0.0,  help="Ramp-down seconds (default: 0)")
    ...
    while True:
        ...
        if now >= report_at:
            ...
            print(f"  {elapsed:5.1f}s | rate={rate:5.1f} sent={sent:5d} "
