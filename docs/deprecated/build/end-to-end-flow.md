...
When PSI exceeds configured threshold (typically 0.25):
...
When the count of labeled predictions reaches the configured threshold (50):
...
```
PsiThresholdBreached (PSI > 0.25) present
AnnotationCountReached (>=50 labeled predictions) present
  ↓
...
```
Initial (0:00)   → 20% canary / remaining stable (2m hold)
Phase 2 (2m)   → 40% canary / remaining stable (2m hold)  
...
Monitoring interval: 10 seconds
  ...
  Value: requests per second over last 30s
  
  If RPS > threshold (configured example: 5 RPS per replica):
...
|-------|-------|-----------|
| `HighLatency` | p99 latency > 1.0 second for 5m | Check serving logs, increase replicas, profile model |
| `PsiThresholdBreached` | PSI > 0.25 (distribution shift) | Expect retraining to trigger once labels are ready |
| `AnnotationCountReached` | 50+ pending predictions labeled | Expect retraining to trigger once drift is detected |
