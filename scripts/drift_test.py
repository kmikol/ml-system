...
def _load_pools() -> tuple[list[bytes], list[bytes]]:
    ...
    Each payload includes the sample UUID, which the serving service stores as
    predictions.uuid.  This enables the annotation job to look up the ground
    truth label from the file-based oracle (uuids.npy + labels.npy).
    """
