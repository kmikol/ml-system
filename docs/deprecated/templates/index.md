...
2. **Drift detection**: Monitoring service computes Population Stability Index (PSI) on recent prediction distributions. When PSI exceeds threshold ({{ .Values.alerts.drift.psiThreshold }}), a drift alert is issued.

3. **Annotation availability**: When sufficient labeled predictions accumulate ({{ .Values.alerts.annotation.countThreshold }}+), an annotation-ready alert is issued.
