...
Condition-based approach (implemented):
  Retraining triggered when: PSI > 0.25 AND annotations >= 50
...
**Progression**:
```
Stage 1 (0-2m)   20% traffic to candidate / remaining to current
Stage 2 (2-4m)   40% traffic to candidate / remaining to current
...
PSI < 0.25   Minor shift  
PSI >= 0.25  Significant shift (alert threshold in this system)
```
...
Condition 1: PsiThresholdBreached
  Detected when: drift_psi_class_distribution > 0.25
  Indicates: evidence of model performance degradation

Condition 2: AnnotationCountReached
  Detected when: annotation_annotated_count >= 50
...
**Mechanism**:
```
KEDA ScaledObject monitors: sum(rate(predict_arrivals_total[30s]))
  Metric: request arrivals per second (30s window)
  
If RPS > threshold (example: 5 RPS per replica):
